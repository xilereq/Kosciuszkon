    ...
    return (
        <div className="space-y-4">
            ...
            <div className="relative">
                <input value={familyName} onChange={e => setFamilyName(e.target.value)} placeholder="Nazwa rodziny" className="w-full p-4 pl-12 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-purple-500" />
                <Shield className="absolute left-4 top-4 text-slate-400" size={20} />
            </div>
            <div className="relative">
                <input value={userName} onChange={e => setUserName(e.target.value)} placeholder="Twoja nazwa" className="w-full p-4 pl-12 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-purple-500" />
                <UserCircle className="absolute left-4 top-4 text-slate-400" size={20} />
            </div>
            <div className="flex gap-3">
                <button onClick={() => handleAction('create')} disabled={loading} className="flex-1 bg-purple-600 text-white font-bold py-4 rounded-2xl hover:bg-purple-700">Utwórz</button>
