...
SMS_SPAM_TEXT = ("URGENT! You have won a 1 week FREE membership in our "
                 "$100,000 Prize Jackpot! Txt the word: CLAIM to No: "
                 "81010")
SMS_HAM_TEXT = "Hey, are we still meeting for lunch today?"

EMAIL_PHISHING_TEXT = ("Dear user, your PayPal account has been "
                       "compromised. Click here to verify your "
                       "identity: http://secure-paypal-update.com")
EMAIL_SAFE_TEXT = ("Attached is the invoice for the Q3 marketing "
                   "campaign. Let me know if you have questions.")


@pytest.fixture(scope="module", autouse=True)
def setup_models():
    assert os.path.exists(os.path.join(ML_MODELS_DIR,
                                       'sms_model.joblib')), \
        "Brak modelu SMS!"
    assert os.path.exists(os.path.join(ML_MODELS_DIR,
                                       'email_model.joblib')), \
...
def test_sms_spam_prediction():
    ...
    explanation = generate_spam_explanation(SMS_SPAM_TEXT, 'sms',
                                            result['confidence'])
...
def test_email_phishing_prediction():
    ...
    explanation = generate_spam_explanation(EMAIL_PHISHING_TEXT,
                                            'email',
                                            result['confidence'])

    print("\n" + "=" * 50)
    print(
        f"🛑 TEST EMAIL PHISHING (Pewność: {result['confidence']:.2%})")
...
def test_training_explanation_false_alarm():
    ...
    print("\n" + "=" * 50)
    print(
        "🎓 TEST TRENINGOWY: Użytkownik FAŁSZYWIE oskarżył dobrą "
        "wiadomość")
